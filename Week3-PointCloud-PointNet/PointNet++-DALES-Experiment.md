...
Tile: `5080_54470_new.ply`（7 类，缺卡车）。真实标签与模型预测高度一致，地面、建筑、植被均几乎无错。

| 真实标签 (GT) | 模型预测 (Pred) | 正确/错误分布 |
|:---:|:---:|:---:|
| ![Block A GT](images/block_a_gt.png) | ![Block A Pred](images/block_a_pred.png) | ![Block A Error](images/block_a_err.png) |
...
| 真实标签 (GT) | 模型预测 (Pred) | 正确/错误分布 |
|:---:|:---:|:---:|
| ![Block B GT](images/block_b_gt.png) | ![Block B Pred](images/block_b_pred.png) | ![Block B Error](images/block_b_err.png) |

> 🔍 **Block B 错误率为什么高达 19.7%？**
>
> 1. **类别混淆严重**：包含轿车（275 点）和卡车（94 点），两者在航空俯视视角下形状相似，大量卡车被误判为轿车；
> 2. **缺少电力线和电线杆**：Block B 不含这两类，而 Block A 包含——说明电力线/电线杆这类**细长结构**的 block 错误率反而低（特征鲜明、易于区分）；
> 3. **建筑占比高**：建筑占 40%（1633/4096），部分建筑边缘点易与地面/植被混淆。
